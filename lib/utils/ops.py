...
def renorm(input, is_training):
    return tf.compat.v1.layers.batch_normalization(input, training=is_training, renorm_momentum=0.9)

def instance_norm(input, is_training):
    """ instance normalization """
    with tf.compat.v1.variable_scope('instance_norm'):
        num_out = input.get_shape()[-1]
        scale = tf.compat.v1.get_variable('scale', [num_out],
                initializer=tf.compat.v1.random_normal_initializer(mean=1.0, stddev=0.02))
        offset = tf.compat.v1.get_variable('offset', [num_out],
...
def pad(input, pad_size):
    ...
    if not pad_size:
        return input
    return tf.pad(tensor=input, paddings=[[0,0],[pad_size, pad_size],[pad_size, pad_size],[0,0]], mode='REFLECT')
